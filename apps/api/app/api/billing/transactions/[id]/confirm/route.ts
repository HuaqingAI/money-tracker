...
function toErrorResponse(
  ...
  logger?.error({ err: error }, 'billing:confirm:error');
...
> {
  ...
  const parsed = confirmTransactionInputSchema.safeParse(body);
  if (!parsed.success) {
    return {
      success: false,
      response: errorResponse(
...
export function POST(
  ...
  return withRequestLogging(request, async ({ logger }) => {
    try {
      ...
      const result = await getConfirmationService().confirmTransaction({
        ...(parsed.categoryId ? { categoryId: parsed.categoryId } : {}),
        transactionId: id,
        userId: user.id,
      });
      return successResponse<ConfirmTransactionResult>(result);
    } catch (error) {
