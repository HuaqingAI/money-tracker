...
- **`database.ts` 手写未 CLI 生成** — 文件头已说明为 baseline；本地 Supabase 启动后运行 `npx supabase gen types typescript --local --schema auth,billing,analytics > packages/shared/types/database.ts` 以完全对齐。
